...
class LogisticRegression(nn.Module):
    ...
    def __init__(self, n_classes, n_features, **kwargs):
        ...
        self.linear.weight = nn.Parameter(torch.zeros(n_classes, n_features))
        self.linear.bias = nn.Parameter(torch.zeros(n_classes))
...
class FeedforwardNetwork(nn.Module):
    def __init__(
        ...
        layer_in =  nn.Linear(n_features, hidden_size)
        layer_in.weight= nn.Parameter(torch.normal(mean=0.01, std=0.01,size=(hidden_size, n_features)))
        layer_in.bias = nn.Parameter(torch.zeros(hidden_size))

        layer_out =  nn.Linear(hidden_size, n_classes)
        layer_out.weight= nn.Parameter(torch.normal(mean=0.01, std=0.01,size=(n_classes, hidden_size)))
        layer_out.bias = nn.Parameter(torch.zeros(n_classes))

        modules.append(layer_in)

        if layers>1:
            for index in range(layers-1):
                layer = nn.Linear(hidden_size, hidden_size)
                layer.weight= nn.Parameter(torch.normal(mean=0.01, std=0.01,size=(hidden_size, hidden_size)))
                layer.bias = nn.Parameter(torch.zeros(hidden_size))
                modules.append(layer)

        modules.append(layer_out)
    ...
    def forward(self, x, **kwargs):
        ...
        for index in range(batch_size):
            ...
            for layer in range(self.layers+1):
                current_model = self.sequential[layer]
                pre_activ = current_model(x_layer)
                if layer<self.layers:
                    pos_activate = self.activation(pre_activ)
                    x_layer = self.dropout(pos_activate)
                else:
                    pred = pre_activ
            out.append(pred)
